...
export default function Careers() {
  ...
  // ✅ Optional: prevent background scroll when any modal is open
  useEffect(() => {
    const isAnyModalOpen = jobOpen || applyOpen;
  ...
  const openJobDetails = (job) => {
    // ensure apply modal is closed if user opens details
    setApplyOpen(false);
    setApplyJob(null);
  ...
  // ✅ SINGLE SOURCE OF TRUTH: always open apply modal from anywhere
  const openApply = (job) => {
    if (!job) return;

    // close details first so they don't overlap
    setJobOpen(false);
    setSelectedJob(null);
  ...
  return (
    <div className="min-h-screen">
            ...
            {filtered.map((job) => (
              ...
              >
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                  ...
                  {/* ✅ APPLY ALWAYS OPENS APPLY MODAL DIRECTLY */}
                  <button
                    type="button"
      ...
      <JobModal
        ...
        onApply={openApply} // ✅ apply from details => opens apply modal directly
      />
      ...
      <ApplyModal
        open={applyOpen}
        setOpen={(v) => {
          // supports your existing ApplyModal API
          if (!v) closeApply();
